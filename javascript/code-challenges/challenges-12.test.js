...
const replaceZeros = (string) => {
  // Solution code here...
  const firstNumber =   /[0]/g ;
  const secNumber =  "zero" ;

  let last = string.replace( firstNumber, secNumber) ;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let variable = /^((\(\d{3}\))|(\d{3}))[ -]?\d{3}[ -]?\d{4}$/;

  if (variable.test(phoneNumber)) {
    return true ;
  } else{
    return false ;
  }
